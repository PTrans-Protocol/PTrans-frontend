...
export async function loadOrCreateWallet(chainId: string, addressPrefix?: string): Promise<OfflineSigner> {
    const mnemonic = loadOrCreateMnemonic()
    const hdPath = makeCosmoshubPath(0);
    const wallet = await Secp256k1HdWallet.fromMnemonic(mnemonic, {hdPaths: [hdPath], prefix: addressPrefix})
    return wallet
}

export async function loadKeplrWallet(keplrConfig: KeplrConfig): Promise<OfflineSigner | Error> {
